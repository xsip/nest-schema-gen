import { NestMongooseGenerator } from './nest-mongoose-generator';
import {
    ResolutionResult,
    ResolvedInterface,
    ResolvedDeclaration,
...
function iface(name: string, fields: ResolvedField[]): ResolvedDeclaration & { kind: 'interface' } {
...
function makeResult(decls: ResolvedDeclaration[], rootName?: string): ResolutionResult {
...
function generate(
    ...
    rootName?: string,
    opts: ConstructorParameters<typeof NestMongooseGenerator>[0] = {}
) {
...
describe('NestMongooseGenerator – primitive @Prop types', () => {
    it.each([
        ...
        const decls = [iface('IFoo', [field('val', { kind: 'primitive', type: prim as any })])];
        const [file] = generate(decls);
