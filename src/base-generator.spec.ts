import { ImportBuilder, BaseGenerator, BaseGeneratorOptions, GeneratedFile } from './base-generator';
import {
    ResolutionResult,
    ResolvedInterface,
    ResolvedDeclaration,
...
class TestGenerator extends BaseGenerator<BaseGeneratorOptions> {
    ...
    public exposedToClassName(name: string) { return this.toClassName(name); }
    public exposedToTsType(type: FieldType, result: ResolutionResult) { return this.toTsType(type, result); }
    public exposedCollectUsedNames(type: FieldType, out: Set<string>) { return this.collectUsedNames(type, out); }
    public exposedCollectUnionDtoNames(type: FieldType, out: string[]) { return this.collectUnionDtoNames(type, out); }
    public exposedIsAllLiterals(types: FieldType[]) { return this.isAllLiterals(types); }
    public exposedBuildLiteralEnumExpr(types: FieldType[]) { return this.buildLiteralEnumExpr(types); }
    public exposedGenerateEnum(decl: Parameters<typeof this.generateEnum>[0]) { return this.generateEnum(decl); }
}

function makeResult(decls: ResolvedDeclaration[] = []): ResolutionResult {
    return {
        declarations: decls,
...
describe('BaseGenerator.generate()', () => {
    ...
    const iface = (name: string): ResolvedDeclaration => ({
        kind: 'interface',
        name,
        fields: [],
        sourcePath: '',
    ...
    const enumDecl = (name: string): ResolvedDeclaration => ({
        kind: 'enum',
        name,
