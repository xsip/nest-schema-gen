...
exports.NestMongooseGenerator = void 0;
const base_generator_1 = require("./base-generator");
class NestMongooseGenerator extends base_generator_1.BaseGenerator {
    constructor(opts = {}) {
        super(opts, {
    ...
    generateInterface(decl, result) {
        ...
        if (this.opts.timestamps)
            schemaOpts.push("timestamps: true");
    ...
    // ── Prop descriptor ───────────────────────────────────────────────────────
    buildPropDescriptor(field, result) {
        const desc = {};
        if (!field.optional)
            desc.required = true;
    ...
    applyTypeToProp(type, desc, result, insideArray = false) {
        switch (type.kind) {
            case "primitive":
            ...
            case "union": {
                ...
                if (sig.length === 0)
                    return;
                if (sig.length === 1) {
                    this.applyTypeToProp(sig[0], desc, result, insideArray);
                    return;
                }
                // All literals → enum
                if (this.isAllLiterals(sig)) {
                    const firstKind = sig[0].value;
    ...
    renderProp(desc, imports) {
        ...
        if (desc.default)
            entries.push(`default: ${desc.default}`);
        if (entries.length === 0)
            return `@Prop()`;
        if (entries.length === 1)
            return `@Prop({ ${entries[0]} })`;
        const inner = entries.map((e) => `  ${e},`).join("\n");
        return `@Prop({\n${inner}\n})`;
    }
    // ── Lean interface ────────────────────────────────────────────────────────
    buildLeanInterface(decl, schemaName, result) {
        const lines = [];
...
exports.NestMongooseGenerator = NestMongooseGenerator;
