...
function iface(name, fields, sourcePath = '') {
...
function makeResult(decls, rootName = decls[0]?.name ?? 'Root') {
...
function generate(decls, rootName, opts = {}) {
    ...
    const gen = new nest_swagger_generator_1.NestSwaggerGenerator(opts);
...
describe('NestSwaggerGenerator – class structure', () => {
    ...
    it('supports legacy dtoSuffix option', () => {
        ...
        const [file] = generate(decls, undefined, { dtoSuffix: 'Data' });
...
describe('NestSwaggerGenerator – primitive fields', () => {
    it.each([
        ...
        const decls = [iface('IFoo', [field('val', { kind: 'primitive', type: primType })])];
        const [file] = generate(decls);
